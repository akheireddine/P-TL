...
void MainKnapsack::initializeInformation(shared_ptr< Evaluator > evaluator){
	...
	Items_information = evaluator->get_item_information();
	...
	Backpack_capacity = evaluator->get_Backpack_capacity();
...
void MainKnapsack::Generate_random_Population(shared_ptr< Evaluator > evaluator, int number_of_individu){
	...
	int n_item = evaluator->get_item_information().size();

	float Backpack_capacity = evaluator->get_Backpack_capacity();

	vector< tuple<float, vector< float> > > Items_information = evaluator->get_item_information();
